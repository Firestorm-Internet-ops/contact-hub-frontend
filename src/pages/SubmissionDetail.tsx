...
import type { Submission, Email } from "../types";
...
const SubmissionDetail = () => {
    ...
    // Listen for email sync events to refresh conversation
    useEffect(() => {
        const handleEmailsSync = async () => {
            if (!submissionId) return;
            try {
                const emailsData = await getEmails(Number(submissionId));
                setEmails(emailsData);
                const subData = await getSubmission(Number(submissionId));
                setSubmission(subData);
            } catch (err) {
                console.error("Failed to refresh emails after sync", err);
            }
        };

        window.addEventListener('emails-synced', handleEmailsSync);
        return () => window.removeEventListener('emails-synced', handleEmailsSync);
    }, [submissionId]);

    // Auto-scroll to latest reply when emails load
    useEffect(() => {
        if (emails.length > 0 && scrollContainerRef.current) {
    ...
    const handleCloseTicket = async () => {
        ...
        try {
            const updated = await updateSubmission(Number(submissionId), { status: 'closed' });
            setSubmission(updated);
            window.dispatchEvent(new Event('submission-updated'));
        } catch (err) {
    ...
    return (
        <div className="flex h-full bg-gray-50">
            ...
            <div className="flex-1 flex flex-col h-full bg-slate-50 relative">
                ...
                {/* Bottom Action Bar */}
                <div className="absolute bottom-6 right-6 lg:right-10 z-20 flex items-center gap-3">
                    {submission.status === 'closed' ? (
                        <div className="bg-gray-100 text-gray-500 rounded-full px-5 py-3 shadow-sm flex items-center space-x-2">
                        ...
                        <>
                            <button
                                ...
                                className="bg-gray-600 hover:bg-gray-700 text-white rounded-full px-5 py-3 shadow-lg flex items-center space-x-2 transition-transform transform hover:scale-105"
                            >
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
